...
function run_runner(runner: Emulator, code: string) {
    try {
        runner.run(code);
        runner.dump(outputFileName)
    } catch (error) {
        assert.fail()
    }
...
function check_equality(code: string, expected: any) {
    const runner = new Emulator()
    ...
    for (var key in expected) {
        ...
        if (['A', 'B', 'C', 'D'].includes(key)) {
            ...
            const actual = runner_get_register(runner, key as "A" | "B" | "C" | "D");
            assert.equal(actual, expected[key]);

        }
        else if (['zero', 'carry'].includes(key)) {

            actual = runner_get_flags(runner, key);
            assert.equal(actual, expected[key]);

        }

        else {
            const expt = expected[key]
            if (Array.isArray(expt)) {
                for (let i = 0; i < expt.length; i++) {

                    actual = runner_load16(runner, parsedRes['labels'][key] + (2 * i));
                    assert.equal(actual, expt[i]);

                }
            }
            else {
                actual = runner_load16(runner, parsedRes['labels'][key]);
            }
        }
...
function runner_load16(runner: Emulator, address: number) {
    return runner.cpu.memory.load16(address);
}

function runner_get_register(runner:Emulator, reg: "A" | "B" | "C" | "D") {
    var regs = { "A": 0, "B": 1, "C": 2, "D": 3 }
    return runner.cpu.gpr[regs[reg]];
}

function runner_get_flags(runner: Emulator, flag: any) {
    //@ts-ignore (this function is kinda dirty, leaving it as is to not break tests)
    return runner.cpu[flag];
}
